...
import { init, load, save, render, parseTrailer, summarize, KINDS } from './core.js';
function usage() {
...
function main() {
    ...
    if (command === 'init') {
        init(root);
        console.log('Created .citeskill/citations.json');
        return;
    }
    if (command === 'add') {
        ...
        if (refType !== 'commit' && refType !== 'pr')
            throw new Error('--ref-type must be commit or pr.');
        const refValue = o.ref ?? (refType === 'commit' ? git(root, 'rev-parse', 'HEAD') : undefined);
        if (!refValue)
            throw new Error('Missing --ref.');
        const entry = { id: required(o, 'id'), kind, name: required(o, 'name'), ref: { type: refType, value: refValue } };
        if (o.url)
            entry.url = o.url;
        if (o.provider)
            entry.provider = o.provider;
        if (o.model)
            entry.model = o.model;
        if (o['evidence-url'])
            entry.evidenceUrl = o['evidence-url'];
        if (o.share !== undefined)
            entry.estimatedShare = Number(o.share);
    ...
    if (command === 'validate') {
        ...
        for (const e of manifest.entries.filter(x => x.ref.type === 'commit')) {
            try {
                git(root, 'cat-file', '-e', `${e.ref.value}^{commit}`);
            }
            catch {
                missing.push(`Commit not found for ${e.id}: ${e.ref.value}`);
            }
        }
        ...
        if (args.includes('--trailers')) {
            ...
            for (let i = 0; i + 1 < log.length; i += 2)
                for (const id of parseTrailer(log[i + 1]))
                    if (!ids.has(id))
                        missing.push(`Unknown trailer ID ${id} in ${log[i]}`);
        }
        if (missing.length)
    ...
    if (command === 'render') {
        ...
        if (o.output) {
            ...
            if ((begin >= 0) !== (finish >= 0) || (begin >= 0 && finish < begin))
                throw new Error('Broken CiteSkill markers in output file.');
    ...
    if (command === 'summary') {
        console.log(JSON.stringify(summarize(load(root)), null, 2));
        return;
    }
    usage();
}
